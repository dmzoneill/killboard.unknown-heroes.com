...
<table class="kb-subtable">
    {section name=opt loop=$options[id].option}
        ...
        <tr><td width="120"><b>{$o.descr}:</b></td><td><input type="checkbox" name="{$o.name}" id="{$o.name}"{if $config->getConfig($o.name)}checked="checked"{/if}></td></tr>
    {/section}
    {section name=opt loop=$options[id].color}
        {assign var="o" value=$options[id].color[opt]}
        <tr><td width="120"><b>{$o.descr}:</b></td><td><input type="edit" name="{$o.name}" id="{$o.name}"{if $config->getConfig($o.name)} value="{$config->getConfig($o.name)}"{/if}></td></tr>
    {/section}
</table>
{/section}
