            ...
            switch ($_POST['add_type'])
            {
                case 0:
                    $sql = "select crp.crp_id as id, crp.crp_name as name
                      from kb3_corps crp
                     where lower( crp.crp_name ) like '%".strtolower($_POST['add_name'])."%'";
                    break;
                case 1:
                    $sql = "select ali.all_id as id, ali.all_name as name
                      from kb3_alliances ali
                     where lower( ali.all_name ) like '%".strtolower($_POST['add_name'])."%'";
                    break;
                case 2:
                    $sql = "select reg_id as id, reg_name as name
                      from kb3_regions
                     where lower( reg_name ) like '%".strtolower($_POST['add_name'])."%'";
                    break;
                case 3:
                    $sql = "select sys_id as id, sys_name as name
                      from kb3_systems
                     where lower( sys_name ) like '%".strtolower($_POST['add_name'])."%'";
                    break;
            }
...
if ($_GET['op'] == "add")
{
    ...
    $html .= "<tr><td width=80><b>Start date:</b></td><td><input type=text name=ctr_started id=ctr_started size=10 maxlength=10 value=\"".date("Y-m-d")."\"> (yyyy-mm-dd)</td></tr>";
