...
{
    unset($_SESSION['kill_export']);
    unset($_POST);
}
elseif ($_REQUEST['sub'] == 'do')
{
    unset($_SESSION['kill_export']['select']);
    $_SESSION['kill_export']['do'] = 1;
}

$html .= "<form id=\"options\" name=\"options\" method=\"post\" action=\"?a=kill_export\">";
$html .= '<input type="hidden" value="" name=""/>';

if ($_POST)
{
    $dir = $_POST['dir'];
    if (!$dir && $_SESSION['kill_export']['dir'])
    {
        $dir = $_SESSION['kill_export']['dir'];
        $ext = $_SESSION['kill_export']['ext'];
    }
    if (!strstr(stripslashes($dir), stripslashes(getcwd())))
    {
        $dir = getcwd().$dir;
    }
    ...
    if (is_dir($dir))
    {
        if (is_writeable($dir))
        {
            ...
            $_SESSION['kill_export']['select'] = 1;
            $_SESSION['kill_export']['dir'] = $dir;
            $_SESSION['kill_export']['ext'] = $ext;
        }
    }
    else
    {
    ...
    if (!isset($_SESSION['kill_export']['to_export']))
    {
        if (ALLIANCE_ID)
        {
            $_SESSION['kill_export']['to_export'] = 'a'.ALLIANCE_ID;
        }
        else
        {
            $_SESSION['kill_export']['to_export'] = 'c'.CORP_ID;
        }
    }
}
elseif (!isset($_SESSION['kill_export']['do']) || !isset($_SESSION['kill_export']['select']))
{
...
if (isset($_SESSION['kill_export']['select']))
{
    if ($_REQUEST['searchphrase'] != "" && strlen($_REQUEST['searchphrase']) >= 3)
    {
        ...
        while ($row = $qry->getRow())
        {
            ...
            {
                case 'pilot':
                    $link = '?a=kill_export&add=p'.$row['plt_id'];
                    $descr = 'Pilot '.$row['plt_name'].' from '.$row['crp_name'];
                    break;
                case 'corp':
                    $link = "?a=kill_export&add=c".$row['crp_id'];
                    $descr = 'Corp '.$row['crp_name'].', member of '.$row['all_name'];
                    break;
                case 'alliance':
                    $link = '?a=kill_export&add=a'.$row['all_id'];
    ...
    if (!$string = $_SESSION['kill_export']['to_export'])
    {
        $string = '';
    }
    $tmp = explode(',', $string);
    ...
    if ($_REQUEST['add'])
    {
        ...
        $_SESSION['kill_export']['to_export'] = implode(',', $conf);
    }
...
if (isset($_SESSION['kill_export']['do']))
{
    if ($string = $_SESSION['kill_export']['to_export'])
    {
    	$klist = new KillList();
    	...
    	foreach ($kills as $id => $timestamp)
    	{
        	$kill = new Kill($id);
            $cnt++;
            $file = $_SESSION['kill_export']['dir'].$cnt.$_SESSION['kill_export']['ext'];
    		$fp = fopen($file, 'w');
    		fwrite($fp, $kill->getRawMail());
    		fclose($fp);
    	}
    	$html .= $cnt.' mails exported<br/>';
        $html .= '<a href="?a=kill_export">Ok</a>';
        unset($_SESSION['kill_export']);
    }
    else
    {
        // nothing to export, retry
        unset($_SESSION['kill_export']['do']);
        $_SESSION['kill_export']['select'] = 1;
        header('Location: ?a=kill_export');
    }
}
